...
def auto_chunk_audio(sender, instance, created, **kwargs):
    ...
    # Ensure the process starts only after the DB transaction is committed
    if created and instance.audio_file and not instance.is_chunked:
        transaction.on_commit(lambda: process_audio_file(instance))


def process_audio_file(instance):
    try:
        ...
        if not os.access(audio_file_path, os.R_OK):
            raise PermissionError(f"File is not readable: {audio_file_path}")
        ...
        instance.status = 'failed'
        instance.save(update_fields=['status'])
        logger.error(f"PermissionError while accessing transcription {instance.id}: {str(e)}")
        ...
        instance.status = 'failed'
        instance.save(update_fields=['status'])
        logger.error(f"Invalid JSON response during chunking transcription {instance.id}: {str(e)}")
